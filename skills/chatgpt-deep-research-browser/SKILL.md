...
The compiled Oracle manifest may carry `gpt-5.6`, model strategy `select`, and
Oracle `extra-high`, alongside `--browser-research deep`. Active Oracle
deliberately skips generic thinking-time selection for this mode because Deep
Research owns its effort flow. Therefore do not claim a visible `Extra High`
selection or verification for Deep Research. It sends no attachment and
performs no app picker or settings action. Remove `--dry-run` only for an
